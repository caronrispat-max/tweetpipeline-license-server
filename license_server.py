...
def admin_page(token: str = "") -> str:
    ...
    users = sb_select("users", {"select": "username,phone,plan,status,license_expiry,device_id,ip_address,created_at", "order": "id.desc", "limit": "50"})
    try:
    ...
    user_rows = "".join(
        f"<tr><td>{esc(u.get('username'))}</td><td>{esc(u.get('phone'))}</td><td>{esc(u.get('plan'))}</td><td>{esc(u.get('status'))}</td><td>{esc(u.get('license_expiry'))}</td><td>{esc(u.get('device_id'))}</td><td>{esc(u.get('ip_address'))}</td></tr>"
        for u in users
    )
    return f"""
    ...
    <body><div class='wrap'><h1>Tweet Pipeline Pro License Server Admin</h1><p>Storage: Supabase | Time: {esc(utc_now())}</p>
    ...
    <div class='card'><h2>Users</h2><table><tr><th>Username</th><th>Phone</th><th>Plan</th><th>Status</th><th>Expiry</th><th>Device</th><th>IP</th></tr>{user_rows}</table></div>
    </div></body></html>
    """
