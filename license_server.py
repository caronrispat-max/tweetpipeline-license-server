...
def health() -> Dict[str, Any]:
    ...
    for table in ["settings", "payments", "users", "manual_requests", "user_configs"]:
        try:
...
# ---------------------------------------------------------------------
# Admin user management + remote customer configuration
# ---------------------------------------------------------------------
def _get_user_by_username(username: str) -> dict[str, Any]:
    rows = sb_select("users", {"select": "*", "username": f"eq.{username.strip()}", "limit": "1"})
    if not rows:
        raise HTTPException(status_code=404, detail="User not found")
    return rows[0]


def _remote_config_for_user(username: str) -> dict[str, Any]:
    rows = sb_select("user_configs", {"select": "config", "username": f"eq.{username.strip()}", "limit": "1"})
    if rows:
...
def _save_remote_config(username: str, config: dict[str, Any]) -> dict[str, Any]:
    payload = {"username": username.strip(), "config": config or {}, "updated_at": utc_now()}
    rows = sb_upsert("user_configs", payload, "username")
    return rows[0] if rows else payload


def _extend_license(username: str, days: int) -> dict[str, Any]:
    user = _get_user_by_username(username)
    now = datetime.now(timezone.utc)
    current = _parse_iso_dt(user.get("license_expiry")) or now
    if current < now:
        current = now
    expiry = (current + timedelta(days=max(1, int(days or 30)))).isoformat(timespec="seconds")
    updated = sb_update("users", {"username": f"eq.{username}"}, {"status": "active", "license_expiry": expiry})[0]
    return updated


@app.get("/api/admin/users")
def admin_users(x_admin_token: Optional[str] = Header(default=None)) -> Dict[str, Any]:
    require_admin(x_admin_token)
    users = sb_select("users", {"select": "id,username,phone,plan,status,license_expiry,device_id,ip_address,created_at,last_login", "order": "id.desc", "limit": "500"})
    configs = _safe_table("user_configs", {"select": "username,config,updated_at", "limit": "500"})
    cfg_map = {c.get("username"): c for c in configs}
    for u in users:
        u["remote_config"] = cfg_map.get(u.get("username"), {}).get("config", {})
    return {"status": "ok", "users": users, "server_time": utc_now()}


@app.post("/api/admin/user-action")
def admin_user_action(payload: AdminUserActionRequest, x_admin_token: Optional[str] = Header(default=None)) -> Dict[str, Any]:
    require_admin(x_admin_token)
    username = payload.username.strip()
    action = payload.action.strip().lower()
    _get_user_by_username(username)
    ...
    elif action in {"cancel", "cancel_subscription"}:
        updates.update({"status": "cancelled", "license_expiry": utc_now()})
    elif action == "extend":
        user = _extend_license(username, int(payload.days or 30))
        return {"status": "ok", "message": f"Extended {username}", "user": user}
    elif action == "reset_device":
        updates["device_id"] = ""
    elif action == "reset_ip":
        updates["ip_address"] = ""
    elif action == "reset_password":
        temp_password = payload.new_password or generate_temp_password()
        updates.update({"password_hash": hash_password(temp_password), "must_change_password": True})
    else:
        raise HTTPException(status_code=400, detail="Unsupported action")
    updated = sb_update("users", {"username": f"eq.{username}"}, updates)[0]
    result = {"status": "ok", "message": f"{action} applied to {username}", "user": updated}
    if temp_password:
        result["temporary_password"] = temp_password
    return result


@app.get("/api/admin/user-config/{username}")
def admin_get_user_config(username: str, x_admin_token: Optional[str] = Header(default=None)) -> Dict[str, Any]:
    require_admin(x_admin_token)
    _get_user_by_username(username)
    return {"status": "ok", "username": username, "config": _remote_config_for_user(username)}


@app.post("/api/admin/user-config/{username}")
def admin_set_user_config(username: str, payload: AdminUserConfigRequest, x_admin_token: Optional[str] = Header(default=None)) -> Dict[str, Any]:
    require_admin(x_admin_token)
    _get_user_by_username(username)
    saved = _save_remote_config(username, payload.config or {})
    return {"status": "ok", "message": "Remote customer config saved", "username": username, "config": saved.get("config", {})}


@app.post("/api/user/config")
def user_config(payload: UserConfigRequest) -> Dict[str, Any]:
    rows = sb_select("users", {"select": "username,status", "token": f"eq.{payload.token}", "limit": "1"})
...
def login(payload: LoginRequest, request: Request) -> Dict[str, Any]:
    ...
    return {
        ...
        "remote_config": _remote_config_for_user(user["username"]),
    }
